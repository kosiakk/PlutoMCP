...
A thin wrapper that makes [Pluto.jl](https://plutojl.org) available to Claude over MCP — so an assistant can author, edit and run a notebook you are watching in your own browser, instead of handing you code to paste in.
...
Julia 1.10 or newer.
The repo is its own single-plugin marketplace, so the Claude Code plugin is two commands:
