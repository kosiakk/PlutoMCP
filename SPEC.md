...
- `read`: snapshot, dependencies (`dependencies=true`: one hop each way, keyed by the variable that connects the cells), wait until nothing is `running`/`queued` or a new error appears, changes since a timestamp including human edits. The one status/wait/diff tool.
